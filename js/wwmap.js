var wwMap = (function () {
  ...
  function updateSocialText() {
    ...
    var twitterText = encodeURIComponent(
      getTranslation('twitter share text ' + selectedSource),
    )
    var otherText = encodeURIComponent(
      getTranslation('other share text ' + selectedSource),
    )
    var title = encodeURIComponent(getTranslation('Africa Water Week'))
    ...
    d3.select('.ss-share-link.ico-facebook').attr(
      'href',
    ...
    d3.select('.ss-share-link.ico-twitter').attr(
      'href',
    ...
    d3.select('.ss-share-link.ico-google').attr(
      'href',
      'https://plus.google.com/share?url=' + encodedUrl,
    )

    d3.select('.ss-share-link.ico-linkedin').attr(
      'href',
    ...
    embedPre.text(
      embedPre
        .text()
        .replace('index.html', 'index.html' + window.location.search),
    )
  }
  ...
  function addLinksToShareButtons() {
    ...
    d3.select('.ss-share-link.ico-embed').on('click', toggleEmbedCode)
  }
  ...
  function updatePersonKey(peopleUnits) {
    ...
    var key_text =
      ' = ' + numberWithCommas(peopleUnits) + ' ' + getTranslation('people')
  ...
  function isDataForCountry(country_code) {
    if (allData.hasOwnProperty(country_code)) {
      if (
        allData[country_code].hasOwnProperty(selectedSource + '_initial') &&
  ...
  function isTargetDataForCountry(country_code) {
    if (allData.hasOwnProperty(country_code)) {
      if (
        allData[country_code].hasOwnProperty(selectedSource + '_pop_current') &&
  ...
  function selectPeopleUnits(number1, number2) {
    ...
    if (maxNumber < 20000) {
      return 1000
    } else if (maxNumber < 200000) {
      return 10000
    } else if (maxNumber < 2000000) {
      return 100000
    } else if (maxNumber < 20000000) {
      return 1000000
    } else {
      return 10000000
    }
  ...
  function updateSelectedBorder() {
    // remove any old selected border
    d3.select('.selected-country-border').attr(
      'class',
      'selected-country-border ' + selectedSource,
    )
  }
  ...
  function addBorderToSelectedCountry() {
    ...
    var selectedBorder = topojson.mesh(
      mapData,
      mapData.objects.subunits,
      function (a, b) {
        return a.id == selectedCountry || b.id == selectedCountry
      },
    )
    mapsvg
      .append('path')
  ...
  function updateColorScale() {
    ...
    colorScale = d3.scale.threshold().domain(colorDomain).range(colorRange)
  }
  ...
  function targetValueForCountry(country_code, year) {
    if (isDataForCountry(country_code) && year > config.thisYear) {
      var initial = allData[country_code][selectedSource + '_initial']
      var increase = allData[country_code][selectedSource + '_increase']
      var thisYearValue = Math.min(
        100,
        initial + increase * (config.thisYear - config.minYear),
      )
      var targetIncrease =
        (100 - thisYearValue) / (config.maxYear - config.thisYear)
  ...
  function updateLegend() {
    ...
    d3.select('#map-legend-label').text(title)
  }
  ...
  function updateMapInfo() {
    ...
    if (selectedSource == 'water') {
      d3.select('#select-water-source').attr(
        'class',
      ...
      d3.select('#select-sanitation-source').attr(
        'class',
        'button source current-source',
      )
      extraSpace = ''
    ...
    d3.select('.map-info > h1 > span.big')
      ...
      .append('strong')
      .text(' ' + config.minYear.toString() + '-' + config.maxYear.toString())
      .append('span')
      .html(extraSpace)
  }
  ...
  function setCountryInfoAccessText() {
    ...
    var percentValue = formatPercent(
      valueForCountry(selectedCountry, selectedYear),
    )
    if (selectedSource == 'water') {
      accessTextPast = getTranslation('of people have access to water - past')
      accessTextFuture = getTranslation(
        'of people have access to water - future',
      )
      targetText = getTranslation('of people need access to water')
    } else {
      accessTextPast = getTranslation(
        'of people have access to sanitation - past',
      )
      accessTextFuture = getTranslation(
        'of people have access to sanitation - future',
      )
    ...
    var percentSpan = accessTextElement
      .append('span')
      .attr('class', 'access-percentage')
      .text(percentValue)
    percentSpan.append('span').attr('class', 'percent-sign').text('%')
    ...
    if (selectedYear > config.thisYear && endTrendValue < 100) {
      targetValue = formatPercent(
        targetValueForCountry(selectedCountry, selectedYear),
      )
      targetTextElement = accessTextElement
        .append('p')
        .attr('class', 'target-increase')
      percentSpan = accessTextElement
        .append('span')
  ...
  function plotAllYearData() {
    ...
    countryInfo.append('h2').text(getCountryName(selectedCountry))
    countryInfo.append('p').attr('id', 'country-info-access-text')
    setCountryInfoAccessText()

    // add the graph div
    var visDiv = countryInfo.append('div').attr('id', 'country-info-graph')
    var visDivInner = visDiv.append('div').attr('class', 'inner')
  ...
  function updateTargetPanel() {
    if (isTargetDataForCountry(selectedCountry)) {
      var popCurrent =
        1000 * allData[selectedCountry][selectedSource + '_pop_current']
      var popUniversal =
        1000 * allData[selectedCountry][selectedSource + '_pop_universal'] -
        popCurrent
      ...
      if (selectedSource == 'water') {
        d3.select('.targets-subtitle').text(
          getTranslation('Total number of new people gaining access to water'),
        )
        d3.select('.currently > .targets-detail').text(
          getTranslation('more people per year will gain access to water'),
        )
      } else {
        d3.select('.targets-subtitle').text(
          getTranslation(
            'Total number of new people gaining access to sanitation',
          ),
        )
        d3.select('.currently > .targets-detail').text(
          getTranslation('more people per year will gain access to sanitation'),
        )
      }
    } else {
  ...
  function updateMapColors() {
    ...
    mapsvg.selectAll('.country').style('fill', function (d) {
  ...
  function loadedDataCallback(error, africa, dataset, langData) {
    ...
    var borders = topojson.mesh(africa, africa.objects.subunits, function (
      a,
      b,
    ) {
      return true
  ...
  function init(mapconfig) {
    ...
    d3.select('#select-water-source').on('click', function (d) {
      setSource('water')
    })
    d3.select('#select-sanitation-source').on('click', function (d) {
      setSource('sanitation')
    })
    d3.select('#reset-button').on('click', reset)

    mapsvg = d3
      .select('#map')
      .insert('svg', 'div.tooltip')
      .attr('width', width)
      .attr('height', height)
      .attr('class', 'map-svg')
